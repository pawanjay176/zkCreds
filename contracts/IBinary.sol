...
// Binary Assets like you want to prove that you have a driving license but don't want to show the number.
contract IBinary is ZkAssetMintable {
    // event UpdateTotalMinted(bytes32 noteHash, bytes noteData);
    address public binaryAddr;
    ...
    constructor(
        ...
        binaryAddr = msg.sender;
    }

    function register(uint24 _proof, bytes calldata _proofData) external {
        require(msg.sender == binaryAddr, "only the owner can register others");
